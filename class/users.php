...
Class Users extends dbobject{
                            ...
                            if($work_day['code'] != "44")
                            {
                                if($result[0]['church_id'] != "99")
                                {
                                    ...
                                    if($_SESSION['role_id_sess'] == 10){
                                        ...
                                        return json_encode(array("response_code"=>0,"response_message"=>"Successful & Ready To Ride"));
                                        
                                    }else{
                          
                                    //update pin missed and last_login
                                    $this->resetpinmissed($username);
                                    return json_encode(array("response_code"=>0,"response_message"=>"Admin Successfully Logged In"));
                                }
                                }
                                else
                                {
    ...
    public function userlist($data)
    {
		...
		$columner = array(
            ...
            array( 'db' => 'username',   'dt' => 8, 'formatter'=>function($d,$row){
                ...
                else if($_SESSION['role_id_sess'] == 10)
                {
                    return  "<button onclick=\"trigUser('".$d."','".$row['user_disabled']."')\" class='btn btn-sm ".$locking_class."'>".$locking."</button>&nbsp;|&nbsp;<a class='btn btn-sm btn-warning'   onclick=\"getModal('setup/user_edit.php?op=edit&username=".$d."','modal_div')\"  href=\"javascript:void(0)\" data-toggle=\"modal\" data-target=\"#defaultModalPrimary\" >Leave</a>";
                }
    ...
    public function saveUser($data)
    {
        ...
        $validation = "";
            ...
            $validation = $this->validate($data,
                    ...
                    array('mobile_phone'=>'Phone Number','sex'=>'Gender','role_id'=>'Role','username'=>'Username','password'=>'Password','firstname'=>'First Name','lastname'=>'Last Name')
                   );
                if(!$validation['error'])
                {
